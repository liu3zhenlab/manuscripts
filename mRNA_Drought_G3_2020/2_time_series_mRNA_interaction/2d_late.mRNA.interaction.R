...
late <- expdesign[expdesign$Day >= 8, ]

### subset (8 time points in each condition)
# minimum total reads
minCounts <- 2*nrow(late)
d2 <- d[, c("Gene", as.character(late$Sample))]
...
sample.info <- data.frame(row.names=late$Sample,
                          day=as.factor(late$Day),
            			  drought=as.factor(late$Condition),
						  batch=as.factor(late$Batch))
...
pdf(file="late.interaction.plotma.pdf",width = 6,height = 5)
plotMA(results(dds, cooksCutoff=F,independentFiltering=T, alpha=0.05), ylim=c(-15,15))
dev.off()

### output
write.table(res2, "late.mRNA.interaction.raw.output.txt", quote=F, row.names=F, sep="\t")
write.table(sig.out.coef, "late.sig.interaction.mRNAs.coefficients.output.txt",
            quote=F, row.names=F, sep="\t")
write.table(sig.out.se, "late.sig.interaction.mRNAs.se.output.txt",
            quote=F, row.names=F, sep="\t")
write.table(sig.out.dispersion, "late.sig.interaction.mRNAs.dispersion.output.txt",
            quote=F, row.names=F, sep="\t")
